    ...
    public class FBasics_RigidbodyMover : FBasics_RigidbodyMoverBase
    {
        ...
        protected override void UpdateMotor()
        {
            ...
            if (moveDir != Vector3.zero)
            {
                ...
                if (RotateInDir)
                {
                    targetRot = Quaternion.LookRotation(Vector3.ProjectOnPlane(CAM.transform.TransformDirection(moveDir), Vector3.up)).eulerAngles;
                    moveDir = Vector3.forward;
                }
                else
                    targetRot = Quaternion.LookRotation(Vector3.ProjectOnPlane(CAM.transform.forward, Vector3.up)).eulerAngles;
