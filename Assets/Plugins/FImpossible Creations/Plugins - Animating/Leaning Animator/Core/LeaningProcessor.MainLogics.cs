...
namespace FIMSpace
{
    public partial class LeaningProcessor
    {
        ...
        internal void UpdateAngularVelocity()
        {
            ...
            {
                float safeDelta = dt; if (safeDelta < 0.01f) safeDelta = 0.01f;
                float nowEulerY = BaseTransform.eulerAngles.y;
                yawAxisAngularVelocity = Mathf.SmoothDamp(yawAxisAngularVelocity, (Mathf.DeltaAngle(nowEulerY, preRot) / safeDelta) * 0.02f, ref upAxisVeloHelper, (1.2f - RootRapidity) * 0.1f, 20f, dt);
                preRot = nowEulerY;
            }
        }
