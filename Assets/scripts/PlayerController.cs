...
public class  PlayerController : MonoBehaviour
{
    ...
    [SerializeField] private Transform RightHnadIK;
    ...
    [SerializeField] public bool HasCombatWeapon;
    [SerializeField] private BonesStimulator rightHandBone;
    [SerializeField] public Transform CombatWeaponContainer;
  
    [SerializeField] private Animator CombatWeaponTrail;
    [SerializeField] private Collider HitBox;



    [Header("BoomRang")]
    [SerializeField] public bool HasBoomrang;
    [SerializeField] private Transform BoomrangRotator;
    [SerializeField] private Transform PlayerCenterPoint;
    [SerializeField] private float  RotatorSpiningSpeed = 5f;
    [SerializeField] private Transform BoomrangTargetToFollow;
    [SerializeField] private Rigidbody BoomrangRigidbody;
    [SerializeField] private float BoomrangRotateSpeed = 5f;
    [SerializeField] private float BoomrangFollowSpeed = 5f;
    [SerializeField] bool ReturnBoomrangToHnad;
    float timerSpining;
    [SerializeField] private bool BoomrangIsShot;
    Quaternion BoomrangRotatorInitialRotation;
    Quaternion BoomrangRigidbodyInitialRotation;

    [Header("Gun")]
    [SerializeField] public bool HasGunWeapon;
    [SerializeField] public Transform GunWeaponContainer;
    [SerializeField] private Transform GunHandIK;
    [SerializeField] private AimIK GunAimIK;
    ...
    void Start()
    {
        ...
        BoomrangRotatorInitialRotation = BoomrangRotator.rotation ;
          BoomrangRigidbodyInitialRotation  = BoomrangRigidbody.rotation;
    ...
    private void FixedUpdate()
    {
        ...
        bulletSpawner.position = bulletSpawnerTarget.position;
        bulletSpawner.rotation = bulletSpawnerTarget.rotation;


        if (BoomrangIsShot && ReturnBoomrangToHnad == false  )
        {
            CombatWeaponTrail.gameObject.SetActive(false);
            BoomrangRotator.gameObject.SetActive(true);
            BoomrangRotator.transform.parent = null;

            BoomrangRotator.transform.position= PlayerCenterPoint.transform.position;

            CombatWeaponContainer.gameObject.SetActive(false);
            BoomrangRotator.Rotate(0, Time.deltaTime * -RotatorSpiningSpeed, 0);



            Vector3 direction = BoomrangTargetToFollow.position - BoomrangRigidbody.transform.position;

            direction.Normalize();
            BoomrangRigidbody.MovePosition(BoomrangRigidbody.transform.position + (direction * BoomrangFollowSpeed * Time.deltaTime));

            BoomrangRigidbody.transform.parent = null;
            BoomrangRigidbody.transform.Rotate(0, Time.deltaTime * BoomrangRotateSpeed, 0);

   
            timerSpining += Time.deltaTime;
            if (timerSpining >= 3)
            {
                BoomrangRotator.transform.parent = PlayerCenterPoint.transform;

                BoomrangRigidbody.transform.GetComponent<Collider>().enabled = false;
                ReturnBoomrangToHnad = true;
                Invoke("StopBoomrangSpining", 1f);
            }
        ...
        float dis = Vector3.Distance(BoomrangRigidbody.transform.position, PlayerCenterPoint.position);

        if (ReturnBoomrangToHnad && dis >0.25f )
        {
            Vector3 direction = CombatWeaponContainer.position - BoomrangRigidbody.transform.position;
    ...
    void Update()
    {
        ...
        if (CombatWeaponContainer.childCount == 0)
        {
            HasBoomrang = false;
            HasCombatWeapon = false;
        }

        if (GunWeaponContainer.childCount == 3)
        {
            HasGunWeapon = false;
        ...
        {
            if (HasGunWeapon == true  )
            {
                fullbodyik.solver.rightHandEffector.target = GunHandIK;
                ...
                GunAimIK.solver.IKPositionWeight = 1;
            }



            if (HasGunWeapon == false)
            {
                fullbodyik.solver.rightHandEffector.target = RightHnadIK;
                fullbodyik.solver.leftHandEffector.positionWeight = 1;
                fullbodyik.solver.leftHandEffector.rotationWeight = 1;
                fullbodyik.solver.IKPositionWeight = 0;
                GunAimIK.solver.IKPositionWeight = 0;
            }

        }


        if (HasBoomrang && Input.GetKeyDown(KeyCode.Mouse0) && BoomrangIsShot ==false)
        {
            BoomrangIsShot = true;

            BoomrangRigidbody.rotation = Quaternion.identity;
            BoomrangRotator.rotation = Quaternion.identity;
        }
        ...
        if (HasGunWeapon && Input.GetKeyDown(KeyCode.Mouse0) && Time.time >= NextTimeToFire)

        {
            ...
            Vector3 dir = (GunTarget.transform.position - bulletSpawner.transform.position).normalized;
        ...
        if (HasCombatWeapon)
        {
            CombatWeaponTrail.gameObject.SetActive(true);
        }
        ...
        if (Input.GetKeyDown(KeyCode.Mouse0) && !FoundPickup && Anime.GetBool("isPunching") == false && canPunch && HasGunWeapon==false)
        {
    ...
    void OnTriggerStay(Collider other)
    {
        ...
        if (other.gameObject.tag == "BOX")
        {
            ...
            arcLuncherCode. Prefab = BoxPrefab;
        }
        ...
        if (other.gameObject.tag == "ball")
        {
            ...
            arcLuncherCode.Prefab = BallPrefab;
        }
    }
    ...
    private bool Onslope()
    {
        ...
        if (Physics.Raycast(rayu, out slopeHit, RayRange))
        {
            ...
            if (slopeHit.transform.CompareTag("slope"))
            {
    ...
    void enableRagdoll()
    {
        ...
        // puppetMaster.pinWeight -= Time.deltaTime * 2; 
    ...
    IEnumerator resetPunch()
    {
        HitBox.enabled = true;
        CombatWeaponTrail.gameObject.SetActive(true);
        CombatWeaponTrail.Play("Swing", 0, 0);
    ...
    void StopBoomrangSpining()
    {

        timerSpining = 0;
        BoomrangIsShot = false;
        ReturnBoomrangToHnad = false;
        BoomrangRigidbody.transform.parent = BoomrangRotator;

         // BoomrangRotator.rotation = BoomrangRotatorInitialRotation;
       

        BoomrangRigidbody.transform.position = BoomrangTargetToFollow.position;
        BoomrangRotator.gameObject.SetActive(false);
        CombatWeaponContainer.gameObject.SetActive(true);
        BoomrangRigidbody.transform.GetComponent<Collider>().enabled = true;
    }

    
  void returnBoomrangToHnad()
    {
